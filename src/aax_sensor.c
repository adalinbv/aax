      ...
      if (dptr)
      {
         ...
         if (dptr_rb)
         {
            ...
            case AAX_SAMPLES:
               ...
               rv *= _aaxRingBufferGetParami(rb, RB_NO_SAMPLES);
               rv += _aaxRingBufferGetParami(rb, RB_OFFSET_SAMPLES);
               break;
            case AAX_BYTES:
            {
               rv = _intBufGetNumNoLock(smixer->play_ringbuffers, _AAX_RINGBUFFER);
               rv *= _aaxRingBufferGetParami(rb, RB_NO_SAMPLES);
               rv += _aaxRingBufferGetParami(rb, RB_OFFSET_SAMPLES);
               rv *= _aaxRingBufferGetParami(rb, RB_BYTES_SAMPLE);
               break;
            }
      ...
      if (dptr)
      {
         ...
         if (rbuf)
         {
            ...
            if (buf)
            {
               ...
               buf->blocksize = 1;
               buf->pos = 0;
               buf->format = _aaxRingBufferGetParami(rb, RB_FORMAT);
               buf->frequency = _aaxRingBufferGetParamf(rb, RB_FREQUENCY);
...
_aaxSensorCreateRingBuffer(_handle_t *handle)
{
   ...
   if (dptr)
   {
      ...
      if (rb)
      {
         ...
         _aaxRingBufferSetFormat(rb, be->codecs, AAX_PCM24S);
         _aaxRingBufferSetParami(rb, RB_NO_TRACKS, info->no_tracks);

         be = handle->backend.ptr;
         min = be->param(handle->backend.handle, DRIVER_MIN_VOLUME);
         max = be->param(handle->backend.handle, DRIVER_MAX_VOLUME);

         _aaxRingBufferSetParamf(rb, RB_VOLUME_MIN, min);
         _aaxRingBufferSetParamf(rb, RB_VOLUME_MAX, max);

         /* Do not alter the frequency at this time, it has been set by
          * aaxMixerRegisterSensor and may have changed in the mean time
*/       _aaxRingBufferSetParamf(rb, RB_FREQUENCY, info->frequency);
/*        */

         /* create a ringbuffer with a but of overrun space */
         _aaxRingBufferSetParamf(rb, RB_DURATION_SEC, delay_sec*1.0f);
         _aaxRingBufferInit(rb, AAX_TRUE);

         /* 
          * Now set the actual duration, this will not alter the allocated
          * space since it is lower that the initial duration.
          */
         _aaxRingBufferSetParamf(rb, RB_DURATION_SEC, delay_sec);
         _aaxRingBufferSetState(rb, RB_STARTED);
      }
      _intBufReleaseData(dptr, _AAX_SENSOR);
   }
