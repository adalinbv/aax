...
_aaxPhasingEffectSet(float val, int ptype, unsigned char param)
{
   ...
   else if (param == AAX_LFO_DEPTH || param == AAX_LFO_OFFSET)
   {
      if (ptype == AAX_SECONDS) {
         rv = (PHASING_MIN + val*PHASING_MAX);
      } else if (ptype == AAX_MILLISECONDS) {
         rv = (PHASING_MIN + val*PHASING_MAX)*1e3f;
...
_aaxPhasingEffectGet(float val, int ptype, unsigned char param)
{
   ...
   {
      if (ptype == AAX_SECONDS) {
         rv = _MINMAX((val - PHASING_MIN)/PHASING_MAX, 0.0f, 1.0f);
      } else if (ptype == AAX_MILLISECONDS) {
         rv = _MINMAX((val*1e-3f - PHASING_MIN)/PHASING_MAX, 0.0f, 1.0f);
      } else if (ptype == AAX_MICROSECONDS) {
         rv = _MINMAX((val*1e-6f - PHASING_MIN)/PHASING_MAX, 0.0f, 1.0f);
      }
   }
   return rv;
}
...
_aaxPhasingEffectMinMax(float val, int slot, unsigned char param)
{
   ...
   {    /* min[4] */                  /* max[4] */
    { { -2.0f,  0.01f,  0.0f, 0.0f  }, {     2.0f,    10.0f,  1.0f,  1.0f } },
