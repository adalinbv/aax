                   ...
                   unsigned int track, void *data)
{
   ...
   do
   {
      ...
      mixfact = distort[AAX_MIX_FACTOR]*distort[AAX_MIX_FACTOR];
      fact = distort[AAX_DISTORTION_FACTOR];
      
      _batch_mul_value(dptr, bps, no_samples, (1.0f+255.0f*fact*fact)*mixfact);
      if (distort[CLIPPING_FACTOR] >= 0.66f) {
         bufCompressHQ(dptr, 0, no_samples);
      } else if (distort[CLIPPING_FACTOR] >= 0.33f) {
         bufCompressFast(dptr, 0, no_samples);
      } else {
         bufCompressValve(dptr, 0, no_samples);
      }
