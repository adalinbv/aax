...
static void
_aaxNoneDriver3dPrepare(UNUSED(void* src), UNUSED(const void *info), UNUSED(float ssv), UNUSED(float sdf), UNUSED(void *fp2dpos), UNUSED( void *fp3d_m))
{
}
...
void
_aaxSoftwareDriver3dPrepare(void* src, const void *info, float ssv, float sdf, void* fp2dpos, void *fp3d_m)
{
   assert(fp2dpos);
   assert(fp3d_m);
   assert(info);
   ...
   _aaxEmitterPrepare3d(src, info, ssv, sdf, fp2dpos, fp3d_m);
}
