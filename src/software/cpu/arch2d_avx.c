...
void
_batch_roundps_avx(void_ptr dst, const_void_ptr src, size_t num)
{
   ...
   if (num)
   {
      ...
      if (i)
      {
         ...
         do
         {
            xmm4 = _mm256_round_ps(_mm256_load_ps((const float*)sptr++), ROUNDING);
            xmm5 = _mm256_round_ps(_mm256_load_ps((const float*)sptr++), ROUNDING);
            xmm6 = _mm256_round_ps(_mm256_load_ps((const float*)sptr++), ROUNDING);
            xmm7 = _mm256_round_ps(_mm256_load_ps((const float*)sptr++), ROUNDING);

            _mm256_store_ps((float*)dptr++, xmm4);
            _mm256_store_ps((float*)dptr++, xmm5);
            _mm256_store_ps((float*)dptr++, xmm6);
            _mm256_store_ps((float*)dptr++, xmm7);
         }
         while(--i);
      }
