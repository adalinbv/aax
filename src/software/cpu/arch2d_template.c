...
void
FN(batch_fmul_value,A)(void* dptr, const void* sptr, unsigned bps, size_t num, float f)
{
   if (!num) return;
...
void
FN(batch_dither,A)(void *data, unsigned new_bps, size_t num)
{
   size_t i = num;
   if (num)
   {
 ...
 */
static inline void
FN(aaxBufResampleDecimate,A)(int32_ptr dptr, const_int32_ptr sptr, size_t dmin, size_t dmax, float smu, float freq_factor)
{
...
#if 0
static inline void
FN(aaxBufResampleNearest,A)(int32_ptr dptr, const_int32_ptr sptr, size_t dmin, size_t dmax, float smu, float freq_factor)
{
   if (freq_factor == 1.0f) {
...
static inline void
FN(aaxBufResampleLinear,A)(int32_ptr dptr, const_int32_ptr sptr, size_t dmin, size_t dmax, float smu, float freq_factor)
{
   int32_ptr s = (int32_ptr)sptr;
   int32_ptr d = dptr;
   int32_t samp, dsamp;
...
static inline void
FN(aaxBufResampleCubic,A)(int32_ptr dptr, const_int32_ptr sptr, size_t dmin, size_t dmax, float smu, float freq_factor)
{
...
void
FN(batch_resample,A)(int32_ptr d, const_int32_ptr s, size_t dmin, size_t dmax, float smu, float fact)
{
   assert(fact > 0.0f);
   assert(d != s);

   if (fact < CUBIC_TRESHOLD) {
      FN(aaxBufResampleCubic,A)(d, s, dmin, dmax, smu, fact);
   }
   else if (fact < 1.0f) {
      FN(aaxBufResampleLinear,A)(d, s, dmin, dmax, smu, fact);
   }
   else if (fact >= 1.0f) {
      FN(aaxBufResampleDecimate,A)(d, s, dmin, dmax, smu, fact);
   } else {
//    FN(aaxBufResampleNearest,A)(d, s, dmin, dmax, smu, fact);
