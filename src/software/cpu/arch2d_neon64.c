...
static inline float
hsum_float32x4_neon64(float32x4_t v)
{
...
static inline float
hsum_float32x4x2_neon64(float32x4x2_t v)
{
   return hsum_float32x4_neon64(vaddq_f32(v.val[0], v.val[1]));
}
...
static inline float32x4x2_t
vmla2q_f32(float32x4x2_t a, float32x4x2_t b, float32x4x2_t c)
{
...
fast_sin8_neon64(float32x4x2_t x)
{
   float32x4x2_t four = vdup2q_n_f32(-4.0f);
   return vmul2q_f32(four, vmla2q_f32(x, vneg2q_f32(x), vabs2q_f32(x)));
...
void
_batch_dc_shift_neon64(float32_ptr d, const_float32_ptr s, size_t num, float offset)
{
   size_t i, step;
...
void
_batch_wavefold_neon64(float32_ptr d, const_float32_ptr s, size_t num, float threshold)
{
   size_t i, step;
...
void
_batch_get_average_rms_neon64(const_float32_ptr s, size_t num, float *rms, float *peak)
{
   size_t step, total;
...
void
_batch_cvtps24_24_neon64(void_ptr dst, const_void_ptr src, size_t num)
{
...
void
_batch_atanps_neon64(void_ptr dptr, const_void_ptr sptr, size_t num)
{
...
void
_batch_roundps_neon64(void_ptr dptr, const_void_ptr sptr, size_t num)
{
   float *d = (float*)dptr;
   float *s = (float*)sptr;
   size_t i, step;
   ...
   step = sizeof(float32x4_t)/sizeof(float32_t);
...
void
_batch_cvt24_ps24_neon64(void_ptr dst, const_void_ptr src, size_t num)
{
...
static void
_batch_iadd_neon64(int32_ptr d, const_int32_ptr src, size_t num)
{
   int32_t *s = (int32_t *)src;
   size_t i, size, step;
...
void
_batch_imadd_neon64(int32_ptr dst, const_int32_ptr src, size_t num, float v, float vstep)
{
   ...
   if (fabsf(v - 1.0f) < LEVEL_96DB && vstep <=  LEVEL_96DB) {
      _batch_iadd_neon64(dst, src, num);
      return;
   }
...
static void
_batch_fadd_neon64(float32_ptr dst, const_float32_ptr src, size_t num)
{
...
void
_batch_fmadd_neon64(float32_ptr dst, const_float32_ptr src, size_t num, float v, float vstep)
{
   ...
   if (fabsf(v - 1.0f) < LEVEL_90DB && !need_step) {
      _batch_fadd_neon64(dst, src, num);
      return;
   }
...
void
_batch_cvt24_16_neon64(void_ptr dst, const_void_ptr src, size_t num)
{
...
void
_batch_cvt16_24_neon64(void_ptr dst, const_void_ptr sptr, size_t num)
{
...
void
_batch_cvt16_intl_24_neon64(void_ptr dst, const_int32_ptrptr src,
...
void
_batch_fmul_value_neon64(void_ptr dptr, const_void_ptr sptr, unsigned bps, size_t num, float f)
{
   if (!num) return;
...
static inline void
_aaxBufResampleDecimate_float_neon64(float32_ptr dptr, const_float32_ptr sptr, size_t dmin, size_t dmax, float smu, float freq_factor)
{
   const_float32_ptr s = sptr;
   float32_ptr d = dptr;
   float samp, dsamp;
...
static inline void
_aaxBufResampleLinear_float_neon64(float32_ptr dptr, const_float32_ptr sptr, size_t dmin, size_t dmax, float smu, float freq_factor)
{
   const_float32_ptr s = sptr;
   float32_ptr d = dptr;
   float samp, dsamp;
...
static inline void
_aaxBufResampleCubic_float_neon64(float32_ptr d, const_float32_ptr s, size_t dmin, size_t dmax, float smu, float freq_factor)
{
...
void
_batch_resample_float_neon64(float32_ptr d, const_float32_ptr s, size_t dmin, size_t dmax, float smu, float fact)
{
   assert(fact > 0.0f);

   if (fact < CUBIC_TRESHOLD) {
      _aaxBufResampleCubic_float_neon64(d, s, dmin, dmax, smu, fact);
   }
   else if (fact < 1.0f) {
      _aaxBufResampleLinear_float_neon64(d, s, dmin, dmax, smu, fact);
   }
   else if (fact >= 1.0f) {
      _aaxBufResampleDecimate_float_neon64(d, s, dmin, dmax, smu, fact);
   } else {
//    _aaxBufResampleNearest_float_neon64(d, s, dmin, dmax, smu, fact);
