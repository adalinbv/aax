...
int main() {
  ...
  unsigned int min_rate, max_rate, rate = 44100;
...
printf("SND_PCM_ACCESS_MMAP_NONINTERLEAVED: %i\n", SND_PCM_ACCESS_MMAP_NONINTERLEAVED);
printf("result: %i\n", result);
  if (result < 0)
    showErrorAndExit(result, "Could not set access method: %s\n");
  result = snd_pcm_hw_params_set_format(handle, params, SND_PCM_FORMAT_S16);
  if (result < 0)
  ...
  result = snd_pcm_hw_params_get_rate_min(params, &min_rate, &dir);
  result = snd_pcm_hw_params_get_rate_max(params, &max_rate, &dir);
  if (result < 0) {
