...
int main(int argc, char **argv)
{
   ...
   do {
      ...
      tracks = channels;
      rbs = _oalRingBufferInitSamples(0, 0, 0, tracks, freq, fmt,
                                             no_samples, 0);
      sduration = _oalRingBufferGetDuration(rbs);

      no_loops = _oalRingBufferGetDuration(rbs)/LOOP_TIME;
      dduration = sduration/no_loops;
      ...
      if (rbs && rb16 && backend && backend->detect())
      {
         ...
         smin = 0;
         dmax = ceil(dduration * freq);
         smax = ceil(sduration * freq);
